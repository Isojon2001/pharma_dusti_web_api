...
function RegistrationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!phone.trim() || phone.trim() === '+992') {
      showErrorModal('Пожалуйста, введите номер телефона');
      return;
    }
    ...
    try {
      ...
      if (err.response?.status === 404) {
        ...
        showErrorModal('Ошибка подключения к серверу. Повторите попытку позже.');
      }
    }
  };
    ...
    <div className="registration gap">
      {showModal && (
        <div className="modal-backdrop">
          <div className="modal">
            <h2>{error}</h2>
            <p>Обратитесь  в Тех Поддержку Дусти Фарма</p>
