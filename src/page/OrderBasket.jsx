...
// Функция для извлечения кода товара из ошибки
const extractProductCodeFromError = (errorMessage) => {
    // Ищем паттерн "failed to fetch product data for code XXX"
    const match = errorMessage.match(/failed to fetch product data for code ([^\s:]+)/i);
    if (match && match[1]) {
        return match[1];
    }
    return null;
...
// Функция для парсинга ошибки о долге из ответа 1С
const parseDebtError = (errorMessage) => {
    if (!errorMessage) return null;
    
    // Сначала пытаемся найти JSON с помощью более точного паттерна
    // Формат может быть: ошибка: 1С вернула статус '...': { "Результат": "...", "Ошибка": "..." }
    // или просто JSON в строке с экранированными символами \r\n
    
    // Ищем JSON объект в сообщении (может быть с экранированными символами)
    const jsonPatterns = [
        /\{[\s\S]*?"Ошибка"[\s\S]*?\}/,
        /\{[\s\S]*?"ошибка"[\s\S]*?\}/,
        /\{[\s\S]*?"Error"[\s\S]*?\}/,
        /\{[\s\S]*?\}/, // Общий паттерн для JSON
    ];
    ...
    for (const pattern of jsonPatterns) {
        ...
        if (jsonMatch) {
            try {
                ...
                // Заменяем экранированные символы перед парсингом
                jsonStr = jsonStr.replace(/\\r\\n/g, ' ').replace(/\\n/g, ' ').replace(/\\r/g, ' ');
                jsonStr = jsonStr.replace(/\r\n/g, ' ').replace(/\n/g, ' ').replace(/\r/g, ' ');
                
                const parsed = JSON.parse(jsonStr);
                
                // Проверяем поле "Ошибка" (может быть на русском или английском)
                if (parsed.Ошибка) {
                    return parsed.Ошибка;
                }
                if (parsed.Error) {
                    return parsed.Error;
                }
    ...
    // Если JSON не найден или не распарсился, ищем текст о долге напрямую
    const lowerMessage = errorMessage.toLowerCase();
    if (lowerMessage.includes('просроченный долг') || 
        lowerMessage.includes('долг') || 
        lowerMessage.includes('задолженность')) {
        
        // Пытаемся извлечь конкретное сообщение о долге из строки
        const debtPatterns = [
            /"Ошибка"\s*:\s*"([^"]+)"/i,
            /"ошибка"\s*:\s*"([^"]+)"/i,
            /"Error"\s*:\s*"([^"]+)"/i,
            /Ошибка["']?\s*:\s*["']([^"']*просроченный[^"']*долг[^"']*)["']/i,
            /["']([^"']*просроченный[^"']*долг[^"']*)["']/i,
            /["']([^"']*долг[^"']*)["']/i,
        ];
        ...
        // Если не нашли конкретное сообщение, возвращаем стандартное
        return 'У данного клиента есть просроченный долг';
    }
    
    return null;
};

// Функция для поиска товара в корзине по коду
const findItemByCode = (code) => {
    return cartItems.find(item => 
        item["Код"] === code || 
        item.productKey === code ||
        item.id === code ||
        String(item["Код"]) === String(code)
    );
};
...
const handleSubmitOrder = async () => {
    ...
    try {
        ...
        console.log('Получено сообщение об ошибке:', errorMessage); // Для отладки
        
        // Проверяем, содержит ли ошибка информацию о долге
        const debtError = parseDebtError(errorMessage);
        console.log('Распарсенная ошибка о долге:', debtError); // Для отладки
        
        if (debtError) {
            // Используем распарсенное сообщение о долге
            setApiErrorMessage(debtError);
            setShowErrorModal(true);
            return;
        }
        
        // Проверяем, содержит ли ошибка информацию о недоступном товаре
        const productCode = extractProductCodeFromError(errorMessage);
        
        if (productCode) {
            // Находим товар в корзине
            const itemToRemove = findItemByCode(productCode);
            
            if (itemToRemove) {
                // Получаем название товара для сообщения
                const productName = itemToRemove["Наименование"] || productCode;
                
                // Удаляем товар из корзины
                const key = itemToRemove.productKey || itemToRemove["Код"] || itemToRemove.id;
                removeFromCart(key);
                
                // Показываем уведомление с предложением повторить попытку
                setApiErrorMessage(
                    `Товар "${productName}" (код: ${productCode}) больше недоступен и был удален из корзины.`
                );
                setShouldRetry(true); // Разрешаем повторную попытку
                setShowErrorModal(true);
                return;
            }
        }
        
        // Если не удалось извлечь код товара, показываем общую ошибку
        const msg = errorMessage || "Произошла ошибка при оформлении заказа";
      ...
      {showErrorModal && (
        <OrderErrorModal
          ...
          onRetry={() => {
            setShowErrorModal(false);
            setShouldRetry(false);
            // Автоматически повторяем попытку оформления заказа
            handleSubmitOrder();
          }}
          showRetry={shouldRetry && cartItems.length > 0}
        />
      )}
